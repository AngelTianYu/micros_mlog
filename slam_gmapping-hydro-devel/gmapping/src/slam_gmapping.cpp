...
void SlamGMapping::startLiveSlam()
{
  ...
  sst_ = node_.advertise<nav_msgs::OccupancyGrid>("map1", 1, true);
  sst1_ = node_.advertise<nav_msgs::OccupancyGrid>("map2", 1, true);
  sst2_ = node_.advertise<nav_msgs::OccupancyGrid>("map3", 1, true);
  sst3_ = node_.advertise<nav_msgs::OccupancyGrid>("map4", 1, true);
  sstm_ = node_.advertise<nav_msgs::MapMetaData>("map_metadata1", 1, true);
  sstm1_ = node_.advertise<nav_msgs::MapMetaData>("map_metadata2", 1, true);
  sstm2_ = node_.advertise<nav_msgs::MapMetaData>("map_metadata3", 1, true);
  sstm3_ = node_.advertise<nav_msgs::MapMetaData>("map_metadata4", 1, true);
  ss_ = node_.advertiseService("dynamic_map1", &SlamGMapping::mapCallback, this);
  ss1_ = node_.advertiseService("dynamic_map2", &SlamGMapping::mapCallback1, this);
  ss2_ = node_.advertiseService("dynamic_map3", &SlamGMapping::mapCallback2, this);
  ss3_ = node_.advertiseService("dynamic_map4", &SlamGMapping::mapCallback3, this);
//  scan_filter_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(node_, "scan", 5);
//  scan_filter_ = new tf::MessageFilter<sensor_msgs::LaserScan>(*scan_filter_sub_, tf_, odom_frame_, 5);
//  scan_filter_->registerCallback(boost::bind(&SlamGMapping::laserCallback, this, _1));

  scan_filter_sub_ = new message_filters::Subscriber<gmapping::MultiScan>(node_, "multiscan", 100);
  scan_filter_ = new tf::MessageFilter<gmapping::MultiScan>(*scan_filter_sub_, tf_, odom_frame_, 10);
  scan_filter_->registerCallback(boost::bind(&SlamGMapping::laserCallback, this, _1));
...
void SlamGMapping::startReplay(const std::string & bag_fname, std::string scan_topic)
{
  ...
  entropy_publisher_ = private_nh_.advertise<std_msgs::Float64>("entropy1", 1, true);
  entropy_publisher1_ = private_nh_.advertise<std_msgs::Float64>("entropy2", 1, true);
  entropy_publisher2_ = private_nh_.advertise<std_msgs::Float64>("entropy3", 1, true);
  entropy_publisher3_ = private_nh_.advertise<std_msgs::Float64>("entropy4", 1, true);
  sst_ = node_.advertise<nav_msgs::OccupancyGrid>("map1", 1, true);
  sst1_ = node_.advertise<nav_msgs::OccupancyGrid>("map2", 1, true);
  sst2_ = node_.advertise<nav_msgs::OccupancyGrid>("map3", 1, true);
  sst3_ = node_.advertise<nav_msgs::OccupancyGrid>("map4", 1, true);
...
*/
  ...
  rosbag::Bag bag;
    ...
    // Store up to 5 messages and there error message (if they cannot be processed right away)
    std::queue<std::pair<gmapping::MultiScan::ConstPtr, std::string> > s_queue;
    foreach(rosbag::MessageInstance const m, viewall)
    {
      tf::tfMessage::ConstPtr cur_tf = m.instantiate<tf::tfMessage>();
      if (cur_tf != NULL) {
      ...
      gmapping::MultiScan::ConstPtr s = m.instantiate<gmapping::MultiScan>();
      if (s != NULL) {
        if (!(ros::Time(s->header.stamp)).is_zero())
        {
          s_queue.push(std::make_pair(s, ""));
...
bool
SlamGMapping::initMappertest(const gmapping::MultiScan::ConstPtr& multiscan)
{
    std::cout << "61...." << "\n";
    laser_frame_ = multiscan->header.frame_id;
    ...
    ident.setIdentity();
    ident.frame_id_ = laser_frame_;
    ident.stamp_ = multiscan->header.stamp;
    ...
    tf::Vector3 v;
    v.setValue(0, 0, 1 + laser_pose.getOrigin().z());
    tf::Stamped<tf::Vector3> up(v, multiscan->header.stamp,
                                        base_frame_);
    try
    {
      tf_.transformPoint(laser_frame_, up, up);
    ...
    std::cout << "65...." << "\n";
    std::cout << multiscan->laser_scans.size() << std::endl;

    sensor_msgs::LaserScan scan = multiscan->laser_scans[0];
    ...
    // Compute the angles of the laser from -x to x, basically symmetric and in increasing order
    laser_angles_.resize(scan.ranges.size());
  //  std::cout << "laser_angles size = " << laser_angles_.size()<< "\n";
    // Make sure angles are started so that they are centered
    std::cout << "667...." << "\n";
    double theta = - std::fabs(scan.angle_min - scan.angle_max)/2;
    std::cout << "668...." << "\n";
    ...
    /// @todo Expose setting an initial pose
    GMapping::OrientedPoint initialPose;
    if(!getOdomPose(initialPose, multiscan->header.stamp))
    {
      ROS_WARN("Unable to determine inital pose of laser! Starting point will be set to zero.");
      initialPose = GMapping::OrientedPoint(0.0, 0.0, 0.0);
    }
...
bool
SlamGMapping::addScantest(const gmapping::MultiScan::ConstPtr& multiscan, GMapping::OrientedPoint& gmap_pose)
{


std::vector<GMapping::RangeReading> reading_;

  if(!getOdomPose(gmap_pose, multiscan->header.stamp))
     return false;

  for (int i=0; i<4; i++)
  {
    sensor_msgs::LaserScan scan;
    scan = multiscan->laser_scans[i];
  ...
  return gsp_->processScantest(reading_);
}

void
SlamGMapping::laserCallback(const gmapping::MultiScan::ConstPtr& multiscan)
{
    timeval startlaser , endlaser;
    long long timelaser;

    gettimeofday(&startlaser , NULL);
  ...
  if(!got_first_scan_)
  {
    timeval startinit , endinit;
    long long timeinit;

    gettimeofday(&startinit , NULL);

    if(!initMappertest(multiscan))
      return;

    gettimeofday(&endinit, NULL);
    timeinit = 1000000*(endinit.tv_sec - startinit.tv_sec) + (endinit.tv_usec - startinit.tv_usec);

    std::cout << "finish initMapper Time = "<<timeinit << "\n";
  ...
  timeval startadd , endadd;
  long long timeadd;

  gettimeofday(&startadd , NULL);

  if(addScantest(multiscan, odom_pose))
  {
    gettimeofday(&endadd, NULL);
    timeadd = 1000000*(endadd.tv_sec - startadd.tv_sec) + (endadd.tv_usec - startadd.tv_usec);

    std::cout << "The addScan Time = "<<timeadd << "\n";
    ...
    if(!got_map_ || (multiscan->header.stamp - last_map_update) > map_update_interval_)
    {
      timeval startupdate, endupdate;
      long long timeupdate;

      gettimeofday(&startupdate, NULL);

      updateMap1(multiscan->laser_scans[0]);
      updateMap2(multiscan->laser_scans[1]);
      updateMap3(multiscan->laser_scans[2]);
      updateMap4(multiscan->laser_scans[3]);

      gettimeofday(&endupdate, NULL);
      timeupdate = 1000000*(endupdate.tv_sec - startupdate.tv_sec) + (endupdate.tv_usec - startupdate.tv_usec);

      std::cout << "The updateMap Time = "<<timeupdate << "\n";

      last_map_update = multiscan->header.stamp;
      ROS_DEBUG("Updated the map");
    }
  } else
    ROS_DEBUG("cannot process scan");

  gettimeofday(&endlaser, NULL);
  timelaser = 1000000*(endlaser.tv_sec - startlaser.tv_sec) + (endlaser.tv_usec - startlaser.tv_usec);

  std::cout << "The laserCallback Time = "<<timelaser << "\n";
}
...
void
SlamGMapping::updateMap1(const sensor_msgs::LaserScan& scan)
{
      ...
      n = n->parent)
  {
    ...
    if(!(n->reading_ty)[0]) // (n->reading_ty)[0]
    {
    ...
    matcher.computeActiveArea(smap, n->pose, &((*((n->reading_ty)[0]))[0])); // &((*n->reading)[0])
    matcher.registerScan(smap, n->pose, &((*((n->reading_ty)[0]))[0])); // &((*((n->reading_ty)[0]))[0])

  }
std::cout<<"testtest3:"<<std::endl;
...
void
SlamGMapping::updateMap2(const sensor_msgs::LaserScan& scan)
{
  ROS_DEBUG("Update map");
  boost::mutex::scoped_lock map_lock (map_mutex_);
  GMapping::ScanMatcher matcher;
      ...
      n = n->parent)
  {
    ...
    if(!(n->reading_ty)[1]) // (n->reading_ty)[0]
    {
      ROS_DEBUG("Reading is NULL");
      continue;
    }
    ...
    matcher.invalidateActiveArea();
    matcher.computeActiveArea(smap, n->pose, &((*((n->reading_ty)[1]))[0])); // &((*n->reading)[0])
    matcher.registerScan(smap, n->pose, &((*((n->reading_ty)[1]))[0])); // &((*((n->reading_ty)[0]))[0])
...
std::cout<<"testtest3:"<<std::endl;
  ...
  sst1_.publish(map_.map);
  sstm1_.publish(map_.map.info);
}

void
SlamGMapping::updateMap3(const sensor_msgs::LaserScan& scan)
{
  ROS_DEBUG("Update map");
  boost::mutex::scoped_lock map_lock (map_mutex_);
  GMapping::ScanMatcher matcher;
      ...
      n = n->parent)
  {
    ...
    if(!(n->reading_ty)[3]) // (n->reading_ty)[0]
    {
      ROS_DEBUG("Reading is NULL");
      continue;
    }
    ...
    matcher.invalidateActiveArea();
    matcher.computeActiveArea(smap, n->pose, &((*((n->reading_ty)[3]))[0])); // &((*n->reading)[0])
    matcher.registerScan(smap, n->pose, &((*((n->reading_ty)[3]))[0])); // &((*((n->reading_ty)[0]))[0])
...
std::cout<<"testtest3:"<<std::endl;
  ...
  sst2_.publish(map_.map);
  sstm2_.publish(map_.map.info);
}

void
SlamGMapping::updateMap4(const sensor_msgs::LaserScan& scan)
{
  ROS_DEBUG("Update map");
  boost::mutex::scoped_lock map_lock (map_mutex_);
  GMapping::ScanMatcher matcher;
...
std::cout<<"testtest3:"<<std::endl;
  ...
  sst3_.publish(map_.map);
  sstm3_.publish(map_.map.info);
}

void
SlamGMapping::updateMaptest(const gmapping::MultiScan::ConstPtr& multiscan)
{
  sensor_msgs::LaserScan scan;
  scan = multiscan->laser_scans[0];
...
//  std::vector<nav_msgs::GetMap::Response> map_vector_;
 ...
 {
  scan = multiscan->laser_scans[ind];
      ...
      n = n->parent)
  {
    ...
    if(!(n->reading_ty)[ind]) // (n->reading_ty)[0]
    {
      ROS_DEBUG("Reading is NULL");
      continue;
    }
    ...
    matcher.invalidateActiveArea();
    matcher.computeActiveArea(smap, n->pose, &((*((n->reading_ty)[ind]))[0])); // &((*n->reading)[0])
    matcher.registerScan(smap, n->pose, &((*((n->reading_ty)[ind]))[0])); // &((*((n->reading_ty)[0]))[0])

    for(int ij=0; ij<40; ij++){
        std::cout<<"ind:"<<ind;
        std::cout<<"reading"<<((*((n->reading_ty)[ind]))[ij])<<std::endl;
    }
  ...
  for(int x=0; x < smap.getMapSizeX(); x++)
  {
    ...
    map_.map.header.stamp = ros::Time::now();
    map_vector_.push_back(map_);
 }
  got_map_ = true;

  //make sure to set the header information on the map
  nav_msgs::GetMap::Response map1 = map_vector_[0];
  nav_msgs::GetMap::Response map2 = map_vector_[0];
  nav_msgs::GetMap::Response map3 = map_vector_[0];
  nav_msgs::GetMap::Response map4 = map_vector_[0];

  map1.map.header.frame_id = tf_.resolve( map_frame_ );
  map2.map.header.frame_id = tf_.resolve( map_frame_ );
  map3.map.header.frame_id = tf_.resolve( map_frame_ );
  map4.map.header.frame_id = tf_.resolve( map_frame_ );

  sst_.publish(map1.map);
  sst1_.publish(map2.map);
  sst2_.publish(map3.map);
  sst3_.publish(map4.map);
  sstm_.publish(map1.map.info);
  sstm1_.publish(map2.map.info);
  sstm2_.publish(map3.map.info);
  sstm3_.publish(map4.map.info);
                          ...
                          nav_msgs::GetMap::Response &res)
{
  boost::mutex::scoped_lock map_lock (map_mutex_);
  nav_msgs::GetMap::Response map1 = map_vector_[0];
  if(got_map_ && map1.map.info.width && map1.map.info.height)
  {
    res = map1;
    return true;
  }
...
bool
SlamGMapping::mapCallback1(nav_msgs::GetMap::Request  &req,
                          nav_msgs::GetMap::Response &res)
{
  boost::mutex::scoped_lock map_lock (map_mutex_);
  nav_msgs::GetMap::Response map2 = map_vector_[1];
  if(got_map_ && map2.map.info.width && map2.map.info.height)
  {
    res = map2;
    return true;
  }
...
bool
SlamGMapping::mapCallback2(nav_msgs::GetMap::Request  &req,
                          nav_msgs::GetMap::Response &res)
{
  boost::mutex::scoped_lock map_lock (map_mutex_);
  nav_msgs::GetMap::Response map3 = map_vector_[2];
  if(got_map_ && map3.map.info.width && map3.map.info.height)
  {
    res = map3;
    return true;
  }
...
bool
SlamGMapping::mapCallback3(nav_msgs::GetMap::Request  &req,
                          nav_msgs::GetMap::Response &res)
{
  boost::mutex::scoped_lock map_lock (map_mutex_);
  nav_msgs::GetMap::Response map4 = map_vector_[3];
  if(got_map_ && map4.map.info.width && map4.map.info.height)
  {
    res = map4;
    return true;
  }
