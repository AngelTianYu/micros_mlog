...
class ScanMatcher{
	public:
		...
		double optimize(OrientedPoint& pnew, const ScanMatcherMap& map, const OrientedPoint& p, const double* readings) const;
                double optimizetest(OrientedPoint& pnew, ScanMatcherMap*& map, const OrientedPoint& init, std::vector<double *> & readings) const;
		double optimize(OrientedPoint& mean, CovarianceMatrix& cov, const ScanMatcherMap& map, const OrientedPoint& p, const double* readings) const;
                ...
                double registerScantest(ScanMatcherMap& map, const OrientedPoint& p, const std::vector<double*> & readings_); // test
                double registerScantesttest(ScanMatcherMap*& maps, const OrientedPoint& p, std::vector<double *> readings);  // test
		...
		void computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p, const double* readings);
                void computeActiveAreatest(ScanMatcherMap& map, const OrientedPoint& p, const std::vector<double*> & readings_); // test
                void computeActiveAreatesttest(ScanMatcherMap*& maps, const OrientedPoint& p, std::vector<double *> readings);  // test
		...
		inline unsigned int likelihoodAndScore(double& s, double& l, const ScanMatcherMap& map, const OrientedPoint& p, const double* readings) const;
                inline unsigned int likelihoodAndScoretest(double& s, double& l, ScanMatcherMap*& maps, OrientedPoint& p, std::vector<double*>& readings) const;
...
inline unsigned int ScanMatcher::likelihoodAndScoretest(double& s, double& l, ScanMatcherMap*& maps, OrientedPoint& p, std::vector<double*>& readings) const{
        using namespace std;
        l=0;
        s=0;

        double s0,s1,s2,s3,l0,l1,l2,l3;

        likelihoodAndScore(s0, l0, maps[0], p, readings[0]);
        likelihoodAndScore(s1, l1, maps[1], p, readings[1]);
        likelihoodAndScore(s2, l2, maps[2], p, readings[2]);
        likelihoodAndScore(s3, l3, maps[3], p, readings[3]);

        s = (s0+s1+s2+s3)/readings.size();
        l = (l0+l1+l2+l3)/readings.size();
