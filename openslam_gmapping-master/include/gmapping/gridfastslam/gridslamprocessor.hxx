...
inline void GridSlamProcessor::scanMatchtest(std::vector<double *> &plainReading_)
{
    // sample a new pose from each scan in the reference
//double * plainReading = plainReading_[0];  // test
    double sumScore=0;
    for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
      OrientedPoint corrected;
      double score, l, s;
      score=m_matcher.optimizetest(corrected, it->maps, it->pose, plainReading_);
      //    it->pose=corrected;
...
//      m_matcher.likelihoodAndScore(s, l, (it->maps)[0], it->pose, plainReading);
      m_matcher.likelihoodAndScoretest(s, l, it->maps, it->pose, plainReading_);
      sumScore+=score;
      it->weight+=l;
      it->weightSum+=l;
...
inline bool GridSlamProcessor::resampletest(std::vector<double*> & plainReading_, int adaptSize,
                                        std::vector<RangeReading*> & reading_){
  ...
  if (m_neff<m_resampleThreshold*m_particles.size()){
    ...
    for (unsigned int i=0; i<m_indexes.size(); i++){
      ...
      node=new	TNode(p.pose, 0, oldNode, 0);
      //node->reading=0;
      (node->reading_ty)[0]= reading_[0];
      (node->reading_ty)[1]= reading_[1];
      (node->reading_ty)[2]= reading_[2];
      (node->reading_ty)[3]= reading_[3];
      //			cerr << "A("<<node->parent->childs <<") " <<endl;
