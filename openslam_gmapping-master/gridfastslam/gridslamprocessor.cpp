...
*/
  GridSlamProcessor::Particle::Particle(ScanMatcherMap*& m):
    maps(m), pose(0,0,0), weight(0), weightSum(0), gweight(0), previousIndex(0){
    node=0;
  }
...
//    std::cout<<"scanmatchermap"<<std::endl;
//    ScanMatcherMap *lmaps;
    lmapty[0] = ScanMatcherMap(Point(xmin+xmax, ymin+ymax)*.5, xmax-xmin, ymax-ymin, delta);
    lmapty[1] = ScanMatcherMap(Point(xmin+xmax, ymin+ymax)*.5, xmax-xmin, ymax-ymin, delta);
    lmapty[2] = ScanMatcherMap(Point(xmin+xmax, ymin+ymax)*.5, xmax-xmin, ymax-ymin, delta);
    lmapty[3] = ScanMatcherMap(Point(xmin+xmax, ymin+ymax)*.5, xmax-xmin, ymax-ymin, delta);
    ScanMatcherMap *lmaps = lmapty;
    std::cout<<"for"<<std::endl;
    for (unsigned int i=0; i<size; i++){
      m_particles.push_back(Particle(lmaps));
//      std::cout<<"particle"<<std::endl;
  ...
  bool GridSlamProcessor::processScantest(const std::vector<RangeReading> & reading_, int adaptParticles){

      timeval startprocess, endprocess;
      long long timeprocess;

      gettimeofday(&startprocess, NULL);


    RangeReading reading = reading_[0];  // test
    std::vector<double *>plainReading_;  // test
    std::vector<RangeReading *>reading_copy_; // test
...
reading = reading_[ii];
    ...
    plainReading[i]=reading[i];
      ...
      plainReading_.push_back(plainReading); // test
      ...
      RangeReading* reading_copy =
              new RangeReading(reading.size(),
                               ...
                               reading.getTime());
      /*
      for(int ind = 0; ind < 640; ind++){
      std::cout<<"reading_copy:"<<(*reading_copy)[ind]<<std::endl;
      }
      for(int ind = 0; ind < 640; ind++){
      std::cout<<"reading:"<<reading[ind]<<std::endl;
      }
      */
      reading_copy_.push_back(reading_copy);
}

      if (m_count>0){
    scanMatchtest(plainReading_);
    if (m_outputStream.is_open()){
      m_outputStream << "LASER_READING "<< reading.size() << " ";
      m_outputStream << setiosflags(ios::fixed) << setprecision(2);
      for (RangeReading::const_iterator b=reading.begin(); b!=reading.end(); b++){
    ...
    resampletest(plainReading_, adaptParticles, reading_copy_);

      } else {
    m_infoStream << "Registering First Scan"<< endl;
    for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
      m_matcher.invalidateActiveArea();
      m_matcher.computeActiveAreatesttest(it->maps, it->pose, plainReading_);
      m_matcher.registerScantesttest(it->maps, it->pose, plainReading_);

      // cyr: not needed anymore, particles refer to the root in the beginning!
      TNode* node=new	TNode(it->pose, 0., it->node,  0);
      //node->reading=0;

      (node->reading_ty)[0]= reading_copy_[0];
      (node->reading_ty)[1]= reading_copy_[1];
      (node->reading_ty)[2]= reading_copy_[2];
      (node->reading_ty)[3]= reading_copy_[3];
      //node->reading= reading_copy;
      it->node=node;
      ...
      for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
    it->previousPose=it->pose;
    std::cout<< "particle weight = "<<it->weight <<std::endl;
      }
    ...
    timeprocess = 1000000*(endprocess.tv_sec - startprocess.tv_sec) + (endprocess.tv_usec - startprocess.tv_usec);

    std::cout << "finish processscan Time = "<<timeprocess << "\n";
