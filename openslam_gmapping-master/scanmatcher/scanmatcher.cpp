...
*/
void ScanMatcher::computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p, const double* readings){
    bool map_activeAreaComputed = false;
	OrientedPoint lp=p;
	lp.x+=cos(p.theta)*m_laserPose.x-sin(p.theta)*m_laserPose.y;
	lp.y+=sin(p.theta)*m_laserPose.x+cos(p.theta)*m_laserPose.y;
	lp.theta+=m_laserPose.theta;
...
//    p1.x = 37;
//    p1.y = 3;
//    AccessibilityState s =map.storage().cellState(p1);
//    std::cout<<"state = " << s << std::endl;
    map_activeAreaComputed=true;
}
void ScanMatcher::computeActiveAreatest(ScanMatcherMap& map, const OrientedPoint& p, const std::vector<double*> & readings_){

    double * readings = readings_[0];
...
void ScanMatcher::computeActiveAreatesttest(ScanMatcherMap*& maps, const OrientedPoint& p, std::vector<double *> readings){
    if (m_activeAreaComputed)
        return;
    ScanMatcher::computeActiveArea(maps[0], p, &(*(readings[0])));
    ScanMatcher::computeActiveArea(maps[1], p, &(*(readings[1])));
    ScanMatcher::computeActiveArea(maps[2], p, &(*(readings[2])));
    ScanMatcher::computeActiveArea(maps[3], p, &(*(readings[3])));
    m_activeAreaComputed=true;
}
...
double ScanMatcher::registerScantest(ScanMatcherMap& map, const OrientedPoint& p, const std::vector<double*> & readings_){
...
double ScanMatcher::registerScantesttest(ScanMatcherMap*& maps, const OrientedPoint& p, std::vector<double *> readings){
    std::cout<<"registerscan"<<std::endl;
    ScanMatcher::registerScan(maps[0], p, &(*(readings[0])));
//    std::cout<<"registerscan2"<<std::endl;
    ScanMatcher::registerScan(maps[0], p, &(*(readings[1])));
//    std::cout<<"registerscan3"<<std::endl;
    ScanMatcher::registerScan(maps[2], p, &(*(readings[2])));
//    std::cout<<"registerscan4"<<std::endl;
    ScanMatcher::registerScan(maps[3], p, &(*(readings[3])));
}
/*
...
double ScanMatcher::optimizetest(OrientedPoint& pnew, ScanMatcherMap*& map, const OrientedPoint& init, std::vector<double *> & readings) const{
    double bestScore=-1;
    OrientedPoint currentPose=init;
    double currentScore1=score(map[0], currentPose, readings[0]);
    double currentScore2=score(map[1], currentPose, readings[1]);
    double currentScore3=score(map[2], currentPose, readings[2]);
    double currentScore4=score(map[3], currentPose, readings[3]);
    double currentScore=(currentScore1 + currentScore2 + currentScore3 + currentScore4)/4;
    double adelta=m_optAngularDelta, ldelta=m_optLinearDelta;
    unsigned int refinement=0;
    enum Move{Front, Back, Left, Right, TurnLeft, TurnRight, Done};
/*	cout << __PRETTY_FUNCTION__<<  " readings: ";
...
//		cout <<  "pose=" << currentPose.x  << " " << currentPose.y << " " << currentPose.theta << endl;
        ...
        do {
            ...
            double localScore1=odo_gain*score(map[0], localPose, readings[0]);
            double localScore2=odo_gain*score(map[1], localPose, readings[1]);
            double localScore3=odo_gain*score(map[2], localPose, readings[2]);
            double localScore4=odo_gain*score(map[3], localPose, readings[3]);
            double localScore=(localScore1 + localScore2 + localScore3 + localScore4)/4;
